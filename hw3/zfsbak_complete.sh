...
create_snapshot() {
    ...
    while [ "$snapshot_count" -gt "$rotation_cnt" ]; do
        ...
        existing_snapshots=$(zfs list -t snapshot -o name -s creation | grep "^${dataset}@${SNAPSHOT_PREFIX}") # find about the dataset now
...
list_snapshots() {
    ...
    echo "$snapshots" | while read -r snapshot; do
        ...
        if [ -n "$dataset" ] && [ "$snapshot_dataset" != "$dataset" ]; then
            index=$((index + 1))
            continue
        fi
...
export_snapshot() {
    ...
    # 检查导出是否成功
    if [ $? -eq 0 ]; then
        echo "Export $target_snapshot to ~/$export_file"
    else
